/******************************************************************************
 *                                                                            *
 * Copyright (C) 2021 by nekohasekai <contact-KNApplication@sekai.icu>             *
...
object PluginManager {
    ...
    fun fetchPlugins() = synchronized(this) {
        if (receiver == null) receiver = KNApplication.application.listenForPackageChanges {
            synchronized(this) {
                receiver = null
                cachedPlugins = null
            }
    ...
    private fun initNative(pluginId: String): InitResult? {
        ...
        var providers = KNApplication.application.packageManager.queryIntentContentProviders(
            Intent(PluginContract.ACTION_NATIVE_PLUGIN, buildUri(pluginId, "com.github.dyhkwong.KNApplication")), flags
        )
            .filter { it.providerInfo.exported }
        if (providers.isEmpty()) {
            providers = KNApplication.application.packageManager.queryIntentContentProviders(
                Intent(PluginContract.ACTION_NATIVE_PLUGIN, buildUri(pluginId, "io.nekohasekai.KNApplication")), flags
            )
                .filter { it.providerInfo.exported }
        }
        if (providers.isEmpty()) {
        ...
        if (providers.size > 1) {
            ...
            KNApplication.application.toast(message)
        ...
        try {
            return initNativeFast(
                KNApplication.application.contentResolver,
                pluginId,
                uri
            )?.let { InitResult(it) }
        } catch (t: Throwable) {
    ...
    private fun initNativeSlow(cr: ContentResolver, pluginId: String, uri: Uri): String? {
        ...
        val pluginDir = File(KNApplication.application.noBackupFilesDir, "plugin")
        (cr.query(
            uri,
    ...
    fun ComponentInfo.loadString(key: String) = when (val value = metaData.getString(key)) {
        is String -> value
//        is Int -> KNApplication.application.packageManager.getResourcesForApplication(applicationInfo)
//            .getString(value)
