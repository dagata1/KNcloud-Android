...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    override fun onCleared() {
        getApplication<KNApplication>().unregisterReceiver(mMsgReceiver)
...
//            }
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            launch(Dispatchers.Main) {
                if (key.isNullOrEmpty()) {
                    getApplication<KNApplication>().toastError(R.string.toast_failure)
                } else {
                    getApplication<KNApplication>().toastSuccess(R.string.toast_success)
    ...
    fun initAssets(assets: AssetManager) {
        viewModelScope.launch(Dispatchers.Default) {
            SettingsManager.initAssets(getApplication<KNApplication>(), assets)
        }
    }
        ...
        override fun onReceive(ctx: Context?, intent: Intent?) {
            when (intent?.getIntExtra("key", 0)) {
                ...
                AppConfig.MSG_STATE_START_FAILURE -> {
                    getApplication<KNApplication>().toastError(R.string.toast_services_failure)
                    isRunning.postValue(false)
                }
